...
type QuotaListPayload = Extract<SessionOutboundMessage, { type: "quota/list/response" }>["payload"];
type QuotaFetchPayload = Extract<
  SessionOutboundMessage,
...
export class DaemonClient {
  ...
  async quotaList(requestId?: string): Promise<QuotaListPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
        type: "quota/list",
      },
      responseType: "quota/list/response",
      timeout: 10000,
    });
  }

  async quotaFetch(providerId: string, requestId?: string): Promise<QuotaFetchPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
        type: "quota/fetch",
      ...
      responseType: "quota/fetch/response",
      timeout: 15000,
    });
  }
